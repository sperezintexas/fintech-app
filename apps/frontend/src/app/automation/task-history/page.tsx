...
export default function TaskHistoryPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    const url = showAll ? "/api/tasks/history" : `/api/tasks/history?date=${date}`;
    fetch(url)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to load task history");
        return res.json();
      })
      .then((data: TaskRun[]) => setRuns(Array.isArray(data) ? data : []))
  ...
  return (
    <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-3xl font-bold text-gray-900">Task run history</h2>
            <p className="text-gray-600 mt-1">
              View tasks that ran (Run now or scheduled) and their status or error. Date filter uses UTC.
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {loading ? (
            ...
            <div className="py-16 px-6 text-center text-gray-600 max-w-md mx-auto">
              <p className="font-medium text-gray-700">No task runs found for the selected {showAll ? "history" : "date"}.</p>
              <p className="mt-2 text-sm">
                Task run history shows tasks that have already run (via &quot;Run now&quot; on the Scheduler or from a schedule).
              </p>
              {!showAll && (
