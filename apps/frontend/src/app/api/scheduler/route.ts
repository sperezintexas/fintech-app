...
async function createRecommendedJobs(): Promise<{ created: number; jobs: string[] }> {
  ...
  // Portfolio-level (accountId: null) so tasks appear in Setup > Scheduled Tasks and run for all accounts where applicable
  ...
  for (const r of recommended) {
    ...
    await jobsColl.insertOne(doc as JobDoc & { _id?: ObjectId });
    created++;
  ...
  try {
    ...
    switch (action) {
      ...
      case "run": {
        ...
        if (BUILT_IN_JOB_NAMES.includes(jobName as "refreshHoldingsPrices" | "deliverAlerts" | "unifiedOptionsScanner")) {
          await runBuiltInJob(jobName as "refreshHoldingsPrices" | "deliverAlerts" | "unifiedOptionsScanner", {
      ...
      case "fixAllRecommendedSchedules": {
        ...
        for (const job of allJobs) {
          ...
          const _jobId = (job as { _id: ObjectId })._id.toString();
          await jobsColl.updateOne(
