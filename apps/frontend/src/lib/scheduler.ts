...
/** Built-in job names callable via internal run-task API (e.g. from Kotlin backend). */
export const BUILT_IN_JOB_NAMES = ["refreshHoldingsPrices", "deliverAlerts", "unifiedOptionsScanner"] as const;
export type BuiltInJobName = (typeof BUILT_IN_JOB_NAMES)[number];

export type RunBuiltInJobOptions = {
  accountId?: string | null;
  config?: Record<string, unknown>;
  /** ISO string; for refreshHoldingsPrices throttle outside market hours. */
  lastRun?: string;
};

/**
 * Run a built-in job by name. Used by POST /api/internal/run-task when backend triggers by jobName.
 * For refreshHoldingsPrices: throttles outside market hours (skip if lastRun < 1 hr ago).
 */
export async function runBuiltInJob(
  jobName: BuiltInJobName,
  options: RunBuiltInJobOptions = {}
): Promise<{ success: boolean; error?: string; result?: unknown }> {
  const { accountId, config: configOverride, lastRun } = options;
  try {
    if (jobName === "refreshHoldingsPrices") {
      const lastRunMs = lastRun ? new Date(lastRun).getTime() : 0;
      const now = Date.now();
      const oneHourMs = 60 * 60 * 1000;
      const isMarketHours = getIsMarketHours();
      if (!isMarketHours && lastRunMs > 0 && now - lastRunMs < oneHourMs) {
        return { success: true, result: { skipped: true, reason: "outside market hours, last run < 1h ago" } };
      }
      const stockResult = await refreshHoldingsPricesStock();
      const optionsResult = await refreshHoldingsPricesOptions();
      return {
        success: true,
        result: { stock: stockResult, options: optionsResult },
      };
    }
    if (jobName === "deliverAlerts") {
      const result = await processAlertDelivery(accountId ?? undefined);
      return { success: true, result };
    }
    if (jobName === "unifiedOptionsScanner") {
      let config = configOverride as import("./unified-options-scanner").UnifiedOptionsScannerConfig | undefined;
      if (accountId) {
        const db = await getDb();
        const strategySettings = await db
          .collection<{ accountId: string; excludeWatchlist?: boolean; scannerConfigs?: import("./unified-options-scanner").UnifiedOptionsScannerConfig }>("strategySettings")
      ...
      const result = await runUnifiedOptionsScanner(accountId ?? undefined, config);
      return { success: true, result };
    }
    return { success: false, error: `Unknown jobName: ${jobName}` };
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    return { success: false, error: msg };
  }
