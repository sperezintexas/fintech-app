...
const DEFAULT_REPORT_TYPES: ReportTypeSeed[] = [
  ...
  { id: "unifiedOptionsScanner", handlerKey: "unifiedOptionsScanner", name: "Unified Options Scanner", description: "Runs Option, Covered Call, Protective Put, and Straddle/Strangle scanners in one job", supportsPortfolio: true, supportsAccount: true, order: 4, enabled: true },
  ...
  { id: "riskScanner", handlerKey: "riskScanner", name: "Risk Scanner", description: "Portfolio risk analysis with Grok (holdings only, no watchlist); creates alerts when risk is high", supportsPortfolio: true, supportsAccount: true, order: 6, enabled: true },
];
...
export async function ensureDefaultReportTypes(db: Awaited<ReturnType<typeof getDb>>): Promise<void> {
  ...
  for (const t of DEFAULT_REPORT_TYPES) {
    ...
    } else if (t.id === "riskScanner") {
      await coll.updateOne(
        { id: t.id },
        { $set: { description: t.description, updatedAt: now } }
      );
    }
  }
}
