...
import { executeTask, upsertReportTaskSchedule, cancelReportTaskSchedule } from "@/lib/scheduler";
    ...
    if (!result.success) {
      if (result.summary) {
        return NextResponse.json({
          success: true,
          message: result.error ?? "Task completed; delivery had issues",
          summary: result.summary,
        });
      }
      return NextResponse.json(
        { success: false, error: result.error ?? "Task failed" },
        { status: 400 }
      );
    }
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const update: Partial<Task> & { updatedAt: string } = { updatedAt: new Date().toISOString() };
    ...
    if (body.config !== undefined) {
      ...
      const existingTask = existing as Task | null;
      const typeDoc = existingTask
        ? await db.collection("reportTypes").findOne({ id: existingTask.jobType })
        : null;
      const handlerKey = (typeDoc as { handlerKey?: string } | null)?.handlerKey ?? body.jobType ?? "";
      try {
        update.config = validateJobConfig(body.jobType ?? existingTask?.jobType ?? "", handlerKey, body.config) as TaskConfig | undefined;
      } catch (err) {
    ...
    if (status === "active" && cron) {
      await upsertReportTaskSchedule(id, cron);
    } else {
      await cancelReportTaskSchedule(id);
    }
